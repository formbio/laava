...
total_row <- agg_ref_type %>%
  summarise(
    `Mapped Reference` = "Total",
    `Assigned Type` = "",
    Counts = sum(Counts, na.rm = TRUE),
    `Counts Excluding Lambda` = sum(`Counts Excluding Lambda`, na.rm = TRUE),
    `Frequency (%, Excluding Lambda)` = sum(`Frequency (%, Excluding Lambda)`, na.rm = TRUE)
  )
...
total_row <- vectypes %>%
  summarise(
    `Assigned Type` = "Total",
    Counts = sum(Counts, na.rm = TRUE),
    `GOI Vector Frequency (%)` = sum(`GOI Vector Frequency (%)`, na.rm = TRUE),
    `Frequency (%, Excluding Lambda)` = sum(`Frequency (%, Excluding Lambda)`, na.rm = TRUE)
  )
...
# Create the flextable
flextable(agg_subtype_with_total %>% select(!pct_total)) %>%
  set_header_labels(
    values = c(
      "Assigned Type",
      "Assigned Subtype",
