...
```{r typetable, message=FALSE, warning=FALSE}
flextable(agg_ref_type %>% select(! c(pct_vector, `Frequency (%)`))) %>%
  set_header_labels(values = c(
    "Mapped Reference", 
    "Assigned Type", 
    "Counts", 
...
vectypes <- agg_ref_type %>%
    filter(reference_label == "vector") %>%
    select(! c(reference_label, `Total Frequency (%)`, `Counts Excluding Lambda`))  # Exclude the columns to hide
...
```{r subtypetable, message=FALSE, warning=FALSE}
flextable(agg_subtype %>% select(!`Total Freq. (%)`)) %>%
  set_header_labels(
    values = c(
      "Assigned Type",
      "Assigned Subtype",
