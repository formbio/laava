...
Term |Definition
:-----------:|----------------------------------------------------
Counts                              |Refers to the total number of reads including lambda
Counts Excluding Lambda             |Refers to the total number of reads excluding lambda
Frequency (%, Excluding Lambda)     |Frequency of reads excluding lambda
GOI Vector Frequency (%)            |Refers to the frequency of reads that align to the GOI transfer vector; excludes reads that align to contaminants (chimeric vector, chimeric non-vector, pRepCap, pHelper, host and unmapped) and lambda
...
In the table below, the distribution of reads mapping to the vector, and other contaminants are described. The table includes the counts (includes lambda), counts excluding lambda and the frequency excluding lambda. 

Note: If lambda spike-in was not performed as part of the library prep process, the values in the counts and counts excluding lambda columns will be identical.

```{r typetable, message=FALSE, warning=FALSE}
# Calculate the total row
total_row <- agg_ref_type %>%
  summarise(
...
In the table below, the distribution of reads mapping to the vector, other-vector and backbone are described. The table includes the counts, frequency excluding lambda, and the GOI vector frequency.

```{r vectypes, message=FALSE, warning=FALSE}
 vectypes <- agg_ref_type %>%
     filter(reference_label == "vector") %>%
     select(! c(reference_label, pct_total, counts_wo_lambda))  # Exclude the columns to hide
...
In the table below, the distribution of reads mapping to the various assigned subtypes are described. The table includes the counts, frequency excluding lambda, and the GOI vector frequency.

```{r subtypetable, message=FALSE, warning=FALSE}
# Calculate the total row
total_row <- agg_subtype %>%
  summarise(
    assigned_type = "Total",
    assigned_subtype = "",
    effective_count = sum(effective_count, na.rm = TRUE),
...
if (exists("df.flipflop") && (nrow(df.flipflop) > 1)) {
  flextable(df.flipflop) %>%
    set_header_labels(values = c("type", "subtype", "leftITR", "rightITR", "count", "base"))
}
```
