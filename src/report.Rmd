...
countreads <- df.read1 %>% dplyr::filter(read_ref_label != "lambda")

# Bar chart
ggplot(countreads, aes(x = read_ref_label, y = freq, fill = read_ref_label)) +
...
# Pie chart
ggplot(countreads, aes(x = "", y = e_count, fill = read_ref_label)) +
...
<!--
TODO XXX - conditional on params$vector_type, show (sc|ss)AAV, then other-vector
-->
...
x.read.repcap <- filter(x.all.read, read_ref_label == 'repcap')
