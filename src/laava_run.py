...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--input-bam",
        required=True,
        help="Input BAM file containing sequencing reads"
    )
    parser.add_argument(
        "--vector-fasta",
        required=True,
    ...
    parser.add_argument(
        "--annotation-bed",
        required=True,
        help="Vector annotation in BED format"
    )
    parser.add_argument(
        "--itr-label",
        required=True,
        help="Label used for ITR in annotation"
    )
    
    # Optional arguments
    parser.add_argument(
        "--packaging-fasta",
        help="Packaging sequence reference in FASTA format"
    )
    parser.add_argument(
        "--host-fasta",
        help="Host genome reference in FASTA format"
    )
    parser.add_argument(
        "--output-dir",
    ...
    parser.add_argument(
        "--sample-name",
        help="Human-readable sample name"
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
...
def run_pipeline(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        vector_type = "sc" if cassette_len <= guess_vector_type_length.SC_MAX_THRESHOLD else "ss"
        ...
        env = os.environ.copy()
        env["PATH"] = f"{Path(__file__).parent}:{env['PATH']}"  # Add src dir to PATH for Python scripts
        ...
        if not mapped_reads.exists():
            raise FileNotFoundError(f"Expected mapped reads file not found: {mapped_reads}")
