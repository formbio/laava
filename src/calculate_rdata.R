...
input_params = list()
input_params$input_prefix = args[1]
input_params$annot_filename = args[2] # ex: annotation.txt
input_params$sample_id = args[3]
input_params$vector_type = args[4]
input_params$flipflop_summary = ''
if (length(args) > 4) {
    input_params$flipflop_summary = args[5]
}
...
annot <- read.table(input_params$annot_filename)
...
x.all.summary <- read_tsv(paste0(input_params$input_prefix, '.summary.csv')) %>% mutate(map_start=map_start0,map_end=map_end1) %>% mutate(SampleID=input_params$sample_id,.before=read_id)
write_tsv(x.all.summary,str_c(c(input_params$input_prefix,".alignments.tsv"), collapse = ""))

x.all.err <- read_tsv(paste0(input_params$input_prefix, '.nonmatch_stat.csv.gz')) %>% mutate(SampleID=input_params$sample_id,.before=read_id)
x.all.read <- read_tsv(paste0(input_params$input_prefix, '.per_read.csv')) %>% mutate(SampleID=input_params$sample_id,.before=read_id)
...
write_tsv(x.err.vector,str_c(c(input_params$input_prefix,".sequence-error.tsv"), collapse = ""))
...
write_tsv(x.all.read,str_c(c(input_params$input_prefix,".readsummary.tsv"), collapse = ""))
...
save.image(file = paste0(input_params$input_prefix, ".Rdata"))
